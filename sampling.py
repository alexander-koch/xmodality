...
def ddim_sample_step(
    module, params, key, x, time, time_next, objective="v", eta=0, **kwargs
):
    log_snr = logsnr_schedule_cosine(time)
    log_snr_next = logsnr_schedule_cosine(time_next)
    c = -jnp.expm1(log_snr - log_snr_next)
    ...
    return jax.lax.cond(
        time_next == 0,
        lambda pred: pred,
        lambda pred: pred * alpha_next + eps * adjusted_sigma + noise * ddim_sigma,
        pred,
    )
